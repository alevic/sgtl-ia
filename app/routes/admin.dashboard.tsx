...
import { user as userTable, vehicle as vehicleTable, transaction as transactionTable } from "@/db/schema";
...
export const loader = async ({ request, params }: { request: Request, params: any }) => {
  const [userRes] = await db.select({ value: count() }).from(userTable);
  const [vehicleRes] = await db.select({ value: count() }).from(vehicleTable);
  const [transactionRes] = await db.select({ value: count() }).from(transactionTable);
        ...
        <div className="xl:col-span-2 space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <QuickAction icon={Plus} label="Nova Viagem" color="blue" onClick={() => navigate('/admin/trips/new')} />
              <QuickAction icon={Users} label="Cadastrar Cliente" color="purple" onClick={() => navigate('/admin/clients/new')} />
              <QuickAction icon={Truck} label="Nova Manutenção" color="orange" onClick={() => navigate('/admin/maintenance/new')} />
              <QuickAction icon={Wallet} label="Lançar Despesa" color="emerald" onClick={() => navigate('/admin/finance/transactions/new')} />
            </div>
          </div>
        </div>
